...
// [`State`](#state).
// 
// All arguments are optional. If both an `owner` and `expression` are
// provided, `state` acts in the second capacity, causing `owner` to become
// stateful; otherwise, `state` simply returns a `StateExpression`. The
// `attributes` parameter may include any of the words defined in
// [`STATE_ATTRIBUTE_MODIFIERS`](#module--constants--state-attribute-modifiers);
// these are encoded into the provided `expression`, and will be used to
// further specify the expressed state’s functionality, or to impose
// constraints on how that state may be used by its owner.
// 
// *See also:*
...
var STATE_ATTRIBUTES = {
    ...
    // ##### versioned
    // 
    // Causes alterations to a state to result in a reflexive transition, with
    // a delta object distinguishing the prior version of the state from its
    // new version. Should also add a history entry wherever appropriate,
