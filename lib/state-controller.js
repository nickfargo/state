...
var StateController = ( function () {
    ...
    //
    // * owner            Object                    = {}
    // * supercontroller  StateController           (optional)
    // * expression       (StateExpression|Object)  (optional)
    // * options          Object                    (optional)
    function StateController ( owner, supercontroller, expression, options ) {
        ...
        var self = this,
            name, root, currentState, currentTransition, finalized,
            subcontrollers,
            defaultSubstate;
        ...
        this.change = function (
            ...
            transition && transition.setCallback( function () {
                var state, pathToState, substate, superstate, regions, i, l;
                ...
                if ( transition ) {
                    currentState = target;
                    finalized = target.isFinal();

                    // Landing on a `concurrent` target spawns an array of
                    // subcontrollers — one for each of `target`’s substates.
                    if ( target.isConcurrent() ) {
                        subcontrollers = [];
                        regions = target.substates( false );
                        for ( i = 0, l = regions.length; i < l; i++ ) {
                            subcontrollers[i] = new StateController(
                                this.owner(), this, regions[i]
                            );
                        }
                    }
        ...
        if ( supercontroller ) {
            ...
            // `controller` method that points directly to this controller,
            // along with all of the members and substates outlined in
            // `expression`.
            ...
            if ( currentState.isAbstract() ) {
                defaultSubstate = currentState.defaultSubstate();
                if ( defaultSubstate ) {
