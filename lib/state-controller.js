...
var StateController = ( function () {
    ...
    // 
    // Returns the `Boolean` result of the named `guard` defined on this state,
    // as evaluated `against` the provided state, or `true` if no guard
    // exists.
    function evaluateGuard ( guard, against ) {
        ...
        for ( key in guard ) if ( O.hasOwn.call( guard, key ) ) {
            ...
            selectors = O.trim( key ).split( rxTrimmedCommaDelimiter );
    ...
    StateController.privileged = {
        ...
        change: function ( setCurrent, setTransition ) {
            ...
            return function (
                ...
                // If `target` is a protostate, then it must be represented
                // within `owner` as a transient virtual state that inherits
                // from the protostate.
