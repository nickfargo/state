...
var StateController = ( function () {
    ...
    var rxChangeExpression  = /^\s*([\-|=]>)\s*(.*)/,
        changeMethodMap     = { '->': 'change', '=>': 'changeTo' };
    ...
    function createAccessor ( owner, name, self ) {
        function accessor ( query ) {
            var fn, current, match, method;

            if ( this === owner ) {
                if ( Z.isFunction( fn = arguments[0] ) ) {
                    return self.change( fn.call( this ) );
                }
                current = self.current();
                if ( arguments.length === 0 ) return current;
                if ( typeof query === 'string'  ) {
                    match = query.match( rxChangeExpression );
                    if ( match ) {
                        method = changeMethodMap[ match[1] ];
                        if ( method ) return current[ method ]( match[2] );
                    }
                }
        ...
        change: function ( setCurrent, setTransition ) {
            ...
            return function (
                ...
                transition.setCallback( function () {
                    ...
                    // For each state from `target` to `root` that records a
                    // deep history, push a new element that points to
                    // `target`.
                ...
                return transition.start.apply( transition, options.args )
                    || target;
            };
        }
    };
