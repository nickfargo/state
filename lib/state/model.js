...
Z.assign( State.prototype, {
    ...
    protostate: function () {
        ...
        for ( protostate = next(); protostate; protostate = next() ) {
            ...
            if ( protostate ) {
                // Before returning the located protostate, memoize subsequent
                // lookups with an instance method that closes over it.
                if ( meta.options.memoizeProtostates ) {
                    this.protostate = function () {
                        if ( protostate.destroyed ) {
                            // If `destroyed` has been set, it means we’re
                            // hanging onto an invalid reference, so clear it
                            // for GC, and relay this invocation back up to
