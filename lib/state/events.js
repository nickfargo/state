...
O.assign( State.prototype, {
    'event addEvent on bind removeEvent off unbind emit trigger': O.noop,

    // #### [once](#state--prototype--once)
    //
    // Like [`addEvent`](#state--privileged--add-event), but causes `fn` to be
    // invoked just once on the next `emit`, and then removed.
    //
    // > [once](/api/#state--methods--once)
    once: function ( eventType, fn, context ) {
        return this.addEvent( eventType, fn, context, 1 );
    }
});
