...
O.assign( State.prototype, {
    ...
    'query match': function (
        ...
        var selectors, args, results, parts, cursor, next, result, i, l, name,
        ...
        // A comma-delimited **compound selector** is split, each component is
        // queried in turn, yielding a composite set of states, which is either
        // returned, or evaluated `against`.
        if ( ~selector.search( rxComma ) ) {
            selectors = O.trim( selector ).split( rxTrimmedCommaDelimiter );
            args = O.slice.call( arguments );
            i = 0; l = selectors.length;
            if ( against ) {
                for ( ; i < l; i++ ) {
                    args[0] = selectors[i];
                    if ( this.query.apply( this, args ) ) return true;
                }
                return false;
            } else {
                results = [];
                for ( ; i < l; i++ ) {
                    args[0] = selectors[i];
                    result = this.query.apply( this, args );
                    result && results.push( result );
                }
                return O.unique( O.flatten( results ) );
            }
        }
