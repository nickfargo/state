...
var StateExpression = ( function () {
    ...
    function interpret ( /*Object*/ map ) {
        ...
        for ( key in map ) if ( O.hasOwn.call( map, key ) ) {
            ...
            // categorization.
                ...
                category =
                    ...
                    O.isPlainObject( value )  ? 'states'  :
                    O.isFunction( value )     ? 'methods' :
                    typeof value === 'string' ? 'events'  :
        ...
        // Guards are represented as a hashmap keyed by selector.
        object = result.guards;
        for ( key in object ) if ( O.hasOwn.call( object, key ) ) {
            value = object[ key ];

            // String values are interpreted as a selector. For the guard to
            // pass, the state being guarded against must match the selector.
            if ( typeof value === 'string' ) {
                value = ( function ( selector ) {
                    return function ( against ) {
                        return this.query( selector, against );
                    };
                }( value ) );
            }

            // Non-object values are coerced to a single-element object, with
            // the value keyed to a catch-all wildcard selector.
